...
var ArchificialsAssessmentV2=(()=>{var l={VERSION:"2.0.0",WORKER_URL:"https://law-firm-ai-scorer-v2.law-firm-ai-scorer.workers.dev",ROOT_ID:"ach-af-v2",BRAND:{name:"Archificials",tagline:"AI Integration for Law Firms",website:"https://www.archificials.com",color:{primary:"#1a1a2e",accent:"#e27308",accentHover:"#c96407",bg:"#f8f9fa",card:"#ffffff",text:"#1a1a2e",textLight:"#6c757d",border:"#e0e0e0",success:"#28a745",progressBg:"#e9ecef"}},TIERS:[{max:40,label:"Significant Opportunity",color:"#e27308"},{max:65,label:"Strong Opportunity",color:"#f4a261"},{max:80,label:"Building Readiness",color:"#2a9d8f"},{max:101,label:"Practice Leader",color:"#264653"}],WEIGHTS:{operational:.35,acquisition:.25,digital:.2,practice_readiness:.2}};var _=[],n={phase:"CORE",slideIndex:0,slides:[],selectedModule:null,moduleName:"",moduleIPrimary:null,moduleISecondary:null,answers:{},scores:null,error:null,slideOffset:0,totalAllSlides:0,isSubmitting:!1};function c(e){Object.assign(n,e),_.forEach(t=>t(n))}function f(e,t){n.answers[e]=t,_.forEach(i=>i(n))}function E(){return n.totalAllSlides||n.slides.length}function A(){return n.slideOffset+n.slideIndex+1}function S(){return n.slides[n.slideIndex]||null}function q(){let e=n.totalAllSlides||n.slides.length;return e===0?0:Math.round((n.slideOffset+n.slideIndex+1)/e*100)}var o=l.BRAND.color;function k(){if(document.getElementById("af-v2-styles"))return;let e=document.createElement("style");e.id="af-v2-styles",e.textContent=`
  ...
  `,s.querySelector("[data-action='retry']")?.addEventListener("click",()=>{s.dispatchEvent(new CustomEvent("af-navigate",{detail:{action:"retry"}}))})}function le(e){s.innerHTML=`
    ...
    <div class="af-footer">&copy; ${new Date().getFullYear()} ${l.BRAND.name}</div>
  `;let t=new Set,i=s.querySelector("[data-action='confirm-module-i']");s.querySelectorAll(".af-option").forEach(a=>{a.addEventListener("click",()=>{let r=a.getAttribute("data-key");if(t.has(r))t.delete(r),a.classList.remove("selected");else{if(t.size>=2)return;t.add(r),a.classList.add("selected")}i.disabled=t.size!==2})}),i.addEventListener("click",()=>{let a=Array.from(t);s.dispatchEvent(new CustomEvent("af-module-i-selected",{detail:{primary:a[0],secondary:a[1]}}))}),s.querySelector("[data-action='prev']")?.addEventListener("click",()=>{s.dispatchEvent(new CustomEvent("af-navigate",{detail:{action:"prev"}}))})}function u(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}function ce(){let e=document.getElementById(l.ROOT_ID);if(!e){console.warn(`[Archificials V2] Mount element #${l.ROOT_ID} not found.`);return}k(),ne(e),c({phase:"CORE",slideIndex:0,slides:m,slideOffset:0,totalAllSlides:0}),p(),ye(e)}function ye(e){e.addEventListener("af-navigate",t=>{let{action:i}=t.detail;i==="next"?he(e):i==="prev"?be(e):i==="retry"&&ve(e)}),e.addEventListener("af-module-i-selected",t=>{let{primary:i,secondary:a}=t.detail;xe(e,i,a)})}function he(e){let t=n.slides[n.slideIndex];if(t&&t.required){let i=n.answers[t.id];if(!i||i==="")return}if(t&&t.type==="email"){let i=n.answers[t.id]||"";if(!Oe(i))return}if(n.slideIndex>=n.slides.length-1){we(e);return}c({slideIndex:n.slideIndex+1}),p(),g()}function be(e){if(n.slideIndex>0)c({slideIndex:n.slideIndex-1}),p(),g();else if(n.phase==="CLOSING"){let t=b(n.selectedModule),i;n.selectedModule==="I"&&n.moduleIPrimary?i=I(n.moduleIPrimary,n.moduleISecondary):i=t.questions,c({phase:"MODULE",slides:i,slideIndex:i.length-1,slideOffset:m.length}),p(),g()}else n.phase==="MODULE"&&(c({phase:"CORE",slides:m,slideIndex:m.length-1,slideOffset:0}),p(),g())}function ve(e){c({error:null,isSubmitting:!1}),c({phase:"CLOSING",slides:y,slideIndex:y.length-1}),p()}function we(e){if(n.phase==="CORE"){let t=n.answers.practice_area,i=Ee(t);if(c({selectedModule:i,moduleName:b(i).name}),i==="I"){c({phase:"MODULE_I_PICKER"}),le(ie()),g();return}let a=b(i),r=m.length+a.questions.length+y.length;c({phase:"MODULE",slides:a.questions,slideIndex:0,slideOffset:m.length,totalAllSlides:r}),p(),g()}else n.phase==="MODULE"?(c({phase:"CLOSING",slides:y,slideIndex:0,slideOffset:n.slideOffset+n.slides.length}),p(),g()):n.phase==="CLOSING"&&Ie(e)}function xe(e,t,i){c({moduleIPrimary:t,moduleISecondary:i}),f("module_i_primary",t),f("module_i_secondary",i);let a=I(t,i),r=m.length+a.length+y.length;c({phase:"MODULE",slides:a,slideIndex:0,slideOffset:m.length,totalAllSlides:r}),p(),g()}function Ee(e){let t=m.find(a=>a.id==="practice_area");if(!t)return"I";let i=t.options.find(a=>(typeof a=="string"?a:a.label)===e);return i&&i.route?i.route:"I"}async function Ie(e){c({phase:"SUBMITTING",isSubmitting:!0}),oe(),g();try{let t={...n.answers,version:l.VERSION,practice_area:n.moduleName,module_key:n.selectedModule,timestamp:new Date().toISOString()},i=await fetch(l.WORKER_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!i.ok){let r=await i.text();throw new Error(`Server error (${i.status}): ${r}`)}let a=await i.json();if(a.error)throw new Error(a.error);c({phase:"RESULTS",scores:a.scores||a,isSubmitting:!1}),re(n.scores),g()}catch(t){console.error("[Archificials V2] Submission error:",t),c({phase:"CLOSING",error:t.message,isSubmitting:!1}),se(t.message||"We were unable to process your assessment. Please try again.")}}function Oe(e){return/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)}function g(){let e=document.getElementById(l.ROOT_ID);e&&e.scrollIntoView({behavior:"smooth",block:"start"})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ce):ce();})();
